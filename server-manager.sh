...
# Agua de Florida Plugin Server Manager
...
online-mode=false
white-list=false
motd=Agua de Florida Plugin Test Server
view-distance=10
...
EOF
    ...
    print_status "Note: Use '/aguadeflorida give @p' for testing commands"
}
...
show_status() {
    ...
    if [[ -f "$SERVER_DIR/plugins/${PLUGIN_NAME}-1.0.0.jar" ]]; then
        print_success "Agua de Florida plugin is installed"
    else
        print_warning "Agua de Florida plugin is not installed"
    fi
}
...
case "${1:-help}" in
    ...
    players)
        ...
        ;;
    help|*)
        echo "Agua de Florida Plugin Server Manager"
        echo ""
